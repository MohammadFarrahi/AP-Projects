...
public:
	...
	std::string get_url_image() { return image_url; }
	std::string get_city_name() { return city; }
	std::string get_hotel_name()  {return name; }
	std::string get_overview(){ return hotel_overview; }
	std::vector<std::string> get_amenities();
	std::map<std::string, std::pair<int, int> > get_rooms_info();
